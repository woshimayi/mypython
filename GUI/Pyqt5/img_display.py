...
class ImageWidget(QtWidgets.QWidget):
    ...
    def setSelected(self):
        ...
        if ImageWidget.prevSelected is not None:
...
class ImageContainer(QtWidgets.QFrame):
    ...
    def __init__(self, widgets=None):
        ...
        self.zoomSlider.valueChanged.connect(self.setItemSize)

        self.item_scrollarea = QtWidgets.QScrollArea()
    ...
    def layout(self):
        ...
        # Can do -1
        num_x = max(math.ceil(w / (self.widget_w + self.asset_space)), 1)
        num_y = math.ceil(len(widgets) / num_x)
        self.item_area.resize(
            w, num_y * (self.widget_h + self.asset_space) + 50)

        main_w = self.item_area.width()
        main_h = self.item_area.height()
        # Can do -1
        num_x = max(math.ceil(main_w / (self.widget_w + self.asset_space)), 1)
        ...
        for i in range(len(widgets)):
            space_x = 0
            if self.auto_space:
                space_x = (main_w - self.asset_space * 2 - num_x *
                           (self.widget_w + self.asset_space)) / num_x
