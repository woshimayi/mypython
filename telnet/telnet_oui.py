...
def do_telnet(Host, username, password, finish, commands):
    ...
    for command in commands:
        ...
        print("zzzzz ", str1.splitlines(False)[4].split('='))
        value = str1.splitlines(False)[4].split('=')
        if value:
            if  '123456' == value[1]:
                print("success")
                # return True
...
if __name__=='__main__':
    ...
    commands = ['mdm getpv InternetGatewayDevice.DeviceInfo.ManufacturerOUI', 'ls']
    #do_telnet(Host, username, password, finish, commands)
    finish = '>'
    #
    ret = do_telnet(Host, username, password, finish, commands)
    if ret:
        time.sleep(100)
    else:
        break
