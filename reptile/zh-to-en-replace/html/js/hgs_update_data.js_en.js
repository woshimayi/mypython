...
function submitTableData(objName) {
	...
	if(found){
		alert("Illegal attack behavior of injecting scripts is found. Log out of the current login and log in again!");
		$("#DoLogout").click();
		return;
	}
		...
		switch(state){
			case 0:
				if(firstDisconnectedPromot){
					...
					/*$("#disconnectedPrompt").html('<h3 class="text-danger">The network has been disconnected! The device can no longer be connected!</h3>' 
...
function loadStaticKeyValue(){
    loadStatickv = hgsUpdateData({type:"GET", url:"/staticInfo"}, function(data){
		...
		if (staticInfo["Region"] =="Sichuan") 
		{
			...
			$("#oldPasswordTitle").text("User initial password");
		...
		hgsUpdateData({type:"GET", url:"/getLogStatus"}, function(data){
			if(!data.isAdmin){
				...
				if (g_staticInfo["Region"] =="Beijing" || g_staticInfo["Region"] =="Jiangsu" || g_staticInfo["Region"] == "JiangsuJK") {
					...
					$("#ServiceList").empty();
					$("#ServiceList").append('<option value="INTERNET">Internet access</option>');
				}
				if (g_staticInfo["Region"] =="Beijing"){
