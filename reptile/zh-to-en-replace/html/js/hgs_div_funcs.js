...
ponInfoCheckout = function (data) {
	...
	objData.FECSupport    = (objData.FECSupport == "1")?"支持":"不支持";
	objData.FECDownstream   = (objData.FECDownstream == "1") ? "开启" : "关闭";
...
/* 	
	$.get("/oltInfo", function(data){
		$("#olt_vendor").text(data.OltVendor?data.OltVendor:"未知");
...
$("#ServiceList").change(function () {
	...
	hgsUpdateData(getWandata, function (data) {
		...
		if (foundTr069 || foundVoip) {
			...
			alert(serviceList.toString() + "业务已经建立，请选择其他业务！");
			if (wanName == "新建WAN连接") {
				$("#ServiceList").val("");
			} else {
	...
	if(webPara["VlanEnabled"])
	{
					...
					for (let index = 0; index < wanList.length; index++) {
						...
						if (lastChar == parseInt(webPara["VlanId"])) {
							if (wanitem.ConnectionType.indexOf("PPPoE")>-1 ) {
								alert("已存在一条PPPoE,VLAN"+webPara["VlanId"]+"的WAN连接");
								return 1;
							}
						}
					}
		...
		if(typeof webPara["MulticastVlan"] != "undefined"){
			...
			if((webPara["MulticastVlan"] < 0 && webPara["MulticastVlan"] != -1) || webPara["MulticastVlan"] > 4094){
				...
				alert("合法范围1-4094,-1或0表示不启用,其余值非法组播VLAN!");
				return 1;
			}
...
wanUpdatePageData = function(respData){
	...
	if ((!g_staticInfo["SupportVoip"]) && (g_staticInfo["Region"] == "Guangxi")) {
		if (webWanCon["ServiceList"] == "INTERNET,VOIP") {
			$("#ServiceList").append('<option value="INTERNET,VOIP"  class="FEATURE_VOIP">上网+语音</option>');
		} else {
...
wanCheckout = function(data){
	...
	innerHtml += '<option value="create_new_wan" >新建WAN连接</option>'
...
wanIpv6InfoCheckout = function(data){
	...
	for(x in data.result){
		...
		{
			data.result[x].IPv6DsliteEnable = (data.result[x].IPv6DsliteEnable == 1)?"是":"否";
...
uplinkCheckin = function(data) {
	...
	if (data.AccessType != "GBE"&& uplinkway == "GBE") {
		//lan上行
		if(confirm("LAN上行切换成光上行，即将重启生效！是否确定【重启设备】?")){
			flag = true;
		}
	} else if (data.AccessType=="GBE"&& uplinkway != "GBE"){
		//GPON上行
		// if(confirm("光上行切换成LAN上行，将会修改管理地址为192.168.5.1，即将重启生效！是否确定【重启设备】?")){
		if(confirm("光上行切换成LAN上行，即将重启生效！重启后lan侧地址会被修改为192.168.100.1！是否确定【重启设备】?")){
			flag = true;
		}
...
function informStu(value) {
	...
	if (0 == InformStatus) {
		value["InformStatus"] = "上报成功";
	}	
	else if (3 == InformStatus) {
		value["InformStatus"] = "上报过程中断";
	}
	else if (2 == InformStatus) {
		value["InformStatus"] = "上报无回应";
	}
	else
	{
		value["InformStatus"] =  "未上报";
	}

	if (1 == ConnectStatus)
	{
		value["ConnectStatus"] =  "RMS发起的远程连接过程成功";
	}
	else if (3 == ConnectStatus)
	{
		value["ConnectStatus"] =  "RMS发起的远程连接过程中断";
	}
	else
	{
		value["ConnectStatus"] =  "未收到远程连接请求";
	}	
}
...
FTPCfgCheckIn = function (data) {
	if (data.FtpEnable) {
		if (isValidLoginPasswd(data.FtpPassword) == false) {
			alert('【登录密码】必须同时包含字母、数字和特殊字符，不含空格！');
			return true;
		}
	...
	for(row in data){
		...
		for(n in vlanBindList){
			...
			if(data[row].mode == 0){
				...
				alert(data[row].port + "在【绑定模式】选定【端口绑定】的情况下,不能设置VLAN绑定的参数 \"" + data[row].vlanBind +"\" !请清空!");
				return true;
			}
			...
			if(2 != vlans.length){
				dg.datagrid('selectRow', row);
				setTimeout(vlanbindUpdate, 1000);
				alert(data[row].vlanBind + "是无效的参数!");
				return true;
			}
			...
			for(i in vlans){
				...
				if(vlan < 1 || vlan > 4094){
					dg.datagrid('selectRow', row);
					setTimeout(vlanbindUpdate, 1000);
					alert(vlan + "是无效的VLAN值，VLAN值必须在[1-4094]之间!");
					return true;
				}
	...
	hgsUpdateData({type:"GET",url:"/wanBindInfo"}, function (data) {
		for (var i = 0; i < Data.rows.length; i++) {
			...
			if (!$.isEmptyObject(data[Data.rows[i].port])) {
				...
				$(vb.target).textbox({prompt:"端口已被绑定！"});
...
vxLanCheckout = function(data){
	...
	for(var i in data){
		if(!data[i].Name){
			...
			data[i].WorkModeText = (data[i].WorkMode == 1)?"桥模式":"路由模式";
...
vxLanCheckin = function(pathStr, data){
	if((($("#VxLanWorkMode").val() == "2")) && ($("#vxLanAddrTypeStatic").is(':checked'))){
		...
		var err1 = strategies.isValidIP(IP, "非法的IP地址");
		var err2 = strategies.isValidIPv6(IP, "非法的IP地址");
		if (!err1) {
			var errmask1 = strategies.isValidMask(subnetmask, "无效的子网掩码");
			var errgateway1 = strategies.isValidIP(gatewayIp, "无效的网关地址");
			if (errmask1) {
				alert(errmask1);
				return;
			}
		...
		if (!err2) {
			var errgateway2 = strategies.isValidIPv6(gatewayIp, "无效的网关地址");
			if (errgateway2) {
				alert(errgateway2);
			}
		}
		...
		errorMsg = strategies.isSameSubnet(subnetmask, 
			"无效参数:IP地址[" + IP + "],子网掩码[" + subnetmask + "],网关[" + gatewayIp + "]不在同一个网段!", 
			[IP, gatewayIp]);
		if(errorMsg){
...
lanInfoCheckout = function(data){
	...
	for (i = 0; i < data.length; i++) {
		if (data[i].MACAddress) {
			lanData.push({LanName:g_homegatewayParas["lanCfg"][data[i].fullPath], 
				Status:data[i].Status=="Up"?"已连接":"未连接", MACAddress:data[i].MACAddress.toUpperCase()});
			lanIf.push(data[i]);
		}else{
			lanSta.push(data[i]);
		}		
	...
	for (i = 0; i < lanIf.length; i++) {
		...
		lanData[i].Status = lanIf[i].Status=="Up"?"已连接":"未连接";
		...
		if (lanIf[i].Status=="Up") {
			...
			switch(lanIf[i].DuplexMode){
				case "Auto":
					lanData[i].DuplexMode = "自动";
					break;

				case "Full":
					lanData[i].DuplexMode = "全双工";
					break;

				case "Half":
					lanData[i].DuplexMode = "半双工";
					break;
			};
		} else {
			lanData[i].MaxBitRate = "自动";
...
urlFilterCheckin = function(objName, tableId, data, postData){
	...
	var msg = strategies.isURL(tableCheckedData.Url, "非法的URL");
	if(msg){
		alert(msg);
		return true;
	}
...
macFilterCheckin = function(objName, tableId, data, postData){
	...
	if(tableCheckedData.SourceMACAddress){
		...
		if(!newStandardMac){
			alert(tableCheckedData.SourceMACAddress + "是非法的源MAC地址");
			return true;
		}
	...
	if(tableCheckedData.DestinationMACAddress){
		...
		if(!newStandardMac){
			alert(tableCheckedData.DestinationMACAddress + "是非法的目的MAC地址");
			return true;
		}
	...
	if((tableCheckedData.SourceMACAddress == "")&&(tableCheckedData.DestinationMACAddress == "")){
		alert("源MAC地址或目的MAC地址至少要填写一个!");
		return true;
	}

	if(tableCheckedData.SourceMACAddress == tableCheckedData.DestinationMACAddress){
		alert("源MAC地址和目的MAC地址不可以相同!");
		return true;
	}
...
ipFilterCheckin = function(objName, tableId, data, postData){
	...
	if(rowData.Name.length > 10){
		alert("【规则名称】长度不能超过10字节!");
		return true;
	}

	var errMsg = "";

	errMsg = strategies.isEmptyOrValidIP(rowData.SourceIPStart, "非法的【源IP(起)】")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isEmptyOrValidIP(rowData.SourceIPEnd, "非法的【源IP(止)】")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isEmptyOrValidIP(rowData.DestIPStart, "非法的【目的IP(起)】")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isEmptyOrValidIP(rowData.DestIPEnd, "非法的【目的IP(止)】")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	...
	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.SourcePortStart, "非法的【源端口(起)】", "1:65536")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.SourcePortEnd, "非法的【源端口(止)】", "1:65536")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.DestPortStart, "非法的【目的端口(起)】", "1:65536")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.DestPortEnd, "非法的【目的端口(止)】", "1:65536")
	if(errMsg){
		alert(errMsg);
		return true;
	}
...
vserverCheckin = function(objName, tableId, data, postData){
	...
	if(rowData.PortMappingDescription.length > 256){
		alert("【备注】长度不能超过256字节!");
		return true;
	}

	var errMsg = "";

	errMsg = strategies.isValidIP(rowData.RemoteHost, "非法的【外网IP】")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isValidIP(rowData.InternalClient, "非法的【内网IP】")
	if(errMsg){
		alert(errMsg);
		return true;
	}

	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.PortMappingLeaseDuration, "非法的【租用秒数】, 合法范围0-86400000", "0:86400000")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.minMaxIntValue(rowData.ExternalPort, "非法的【外网端口】, 合法范围1-65555", "1:65535")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.minMaxIntValue(rowData.InternalPort, "非法的【内网端口】, 合法范围1-65555", "1:65535")
	if(errMsg){
		alert(errMsg);
		return true;
	}
...
function intRangeCheck(max, min, range){
	var arr = range.split('-');
	var errMsg = "";
	var errMsg1 = "非法的【MAX】，请输入值为"+range+"之间的整数";
	var errMsg2 = "非法的【Min】，请输入值为"+range+"之间的整数";
...
qosClassificationTypeCheckin = function (objName, tableId, data, postData) {
	...
	switch (tableCheckedData.Type) {
		case "WANInterface":
			...
			if (!foundmax) {
				errMsg1 = "【MAX】不合法，其下拉框中已列出全部有效值，请点击下拉框并从中选择！"
			}
			if (!foundmin) {
				errMsg2 = "【MIN】不合法，其下拉框中已列出全部有效值，请点击下拉框并从中选择！"
			}
			if (max != min) {
				errMsg1 = "匹配类型为WANInterface时，【min】和【max】值应一致"
			}
			break;
		case "LANInterface":
			...
			if (max != min) {
				errMsg1 = "匹配类型为LANInterface时，【min】和【max】值应一致"
			}
			break;
		case "SMAC":
		case "DMAC":
			...
			errMsg1 = strategies.isValidMacAddr(max, "非法的【MAX】MAC地址");
			errMsg2 = strategies.isValidMacAddr(min, "非法的【Min】MAC地址");
			if (!errMsg1 && !errMsg2) {
				var arr1 = max.split(':');
				var arr2 = min.split(':');
				for (var i = 0; i < arr1.length; i++) {
					...
					if (arr1[i] < arr2[i]) {
						errMsg1 = "【MAX】MAC地址不得小于【Min】MAC地址";
					}
				}
			}
			break;
		case "SIP":
		case "DIP":
			errMsg1 = strategies.isEmptyOrValidIP(max, "非法的【MAX】IP地址");
			errMsg2 = strategies.isEmptyOrValidIP(min, "非法的【Min】IP地址");
			err1 = strategies.isValidIPv6(max, "非法的【MAX】IP地址");
			err2 = strategies.isValidIPv6(min, "非法的【Min】IP地址");
			...
			if (!errMsg1 && !errMsg2) {
				...
				if ((!errMsg1 || !err1) && (!errMsg2 || !err2)) {
					errMsg1 = "【MAX】和【Min】应同为IPv4或IPv6地址";
				}
			}
			break;
		case "SPORT":
		...
		case "IPVERSION":
			if (max != min) {
				errMsg1 = "匹配类型为EtherType时，【min】和【max】值应一致"
			}
			break;	
		default:
			break;
...
function qosClassificationTypeEditRow(index, checkRow){
	...
	$(ed.target).combobox({
		...
		onChange:function (newValue, oldValue) {
			...
			if (!found) {
				alert("下拉框中已列出全部有效值，请点击下拉框并从中选择！");
		...
		case 3:
			...
			for (var i = 0; i < speed.length; i++) {
				...
				if (element.length != 2) {
					...
					if (ip.length != 2) {
						alert("ip不合法，应该为ip1-ip2地址段！");
					} else {						
						var err1 = strategies.isValidIP(ip[0],"ip地址不合法！");
						var err2 = strategies.isValidIPv6(ip[0],"ip地址不合法！");
						var err3 = strategies.isValidIP(ip[1],"ip地址不合法！");
						var err4 = strategies.isValidIPv6(ip[1],"ip地址不合法！");
...
rebootCheckIn = function(data){
	if(confirm("即将重启设备!重启设备大约需要50秒钟。是否确定【重启设备】?")){
		return;
	}
...
rebootResp = function(data){
	if(0 == data.retCode){
		...
		//alert("正在重启设备,请耐心等待40秒钟!");
	}else{
		alert("重启设备失败!错误码:" + data.retCode);
	}
}

restoreDefCheckIn = function(data){
	if(confirm("恢复默认配置会导致重启设备!重启设备大约需要50秒钟。是否确定【恢复默认配置】?")){
		return;
	}
...
restoreDefResp = function(data){
	if(0 == data.retCode){
		alert("恢复默认配置成功!正在重启设备,请耐心等待40秒钟!");
	}else{
		alert("恢复默认配置失败!错误码:" + data.retCode);
	}
}

restoreFacCheckIn = function(data){
	if(confirm("恢复出厂设置会导致重启设备!重启设备大约需要40秒钟。是否确定【恢复出厂设置】?")){
		return;
	}
...
restoreFacResp = function(data){
	if(0 == data.retCode){
		alert("恢复出厂设置成功!正在重启设备,请耐心等待40秒钟!");
	}else{
		alert("恢复出厂设置失败!错误码:" + data.retCode);
	}
}
/*----------------------------------------------------------------------------*/
...
staticRouteFullInfoCheckout = function(data){
	...
	for(var i in routeInfo){
		routeInfo[i].Enable = (routeInfo[i].Enable == "0") ?"否":"是";
...
staticRouteCheckin = function(objName, data){
	...
	for(var i in data){
		...
		errMsg = strategies.isValidIP(data[i].DestIPAddress, data[i].DestIPAddress + "是非法的【目的IP】")
		if(errMsg){
			alert(errMsg);
			return;
		}
		errMsg = strategies.isValidMask(data[i].DestSubnetMask,  data[i].DestSubnetMask + "是非法的【目的掩码】")
		if(errMsg){
			alert(errMsg);
			return;
		}
...
staticIPv6RouteCheckin = function(objName, data){
	...
	for(var i in data){
		...
		errMsg = strategies.isValidIPv6Prefix(data[i].DestIPPrefix, data[i].DestIPPrefix + "是非法的【目的IPv6前缀】")
		if(errMsg){
	...
	for(i in data){
		if (data[i].fullPath) {
			if (data[i].fullPath == fullPath_2g) {
				...
				else
				{
					if(data[i].SSID){
						if(data[i].SSID.indexOf("CMCC-") == 0){
							$("#ssidCmcc_ssidpwd_2g").append('<span>CMCC-</span><input type="input" class="col-5" maxlength="27" hgs_key="SSID">(长度:1~32)');
							data[i].SSID = data[i].SSID.substr("CMCC-".length);
							jsonData_2g = data[i];
						}else{
							$("#ssidCmcc_ssidpwd_2g").append('<input type="input" class="col-5" maxlength="32" hgs_key="SSID">(长度:1~32)')
							jsonData_2g = data[i];
						}
					}else{
						$("#ssidCmcc_ssidpwd_2g").append('<span>CMCC-</span><input type="input" class="col-5" maxlength="27" hgs_key="SSID">(长度:1~32)');
				...
				if (g_staticInfo["Region"] == "Zhejiang" || g_staticInfo["Region"] == "Sichuan") {
					$("#ssidCmcc_ssidpwd_5g").append('<input type="input" class="col-5" maxlength="32" hgs_key="SSID_5g">(长度:1~32)')
				...
				else
				{
					if(data[i].SSID){
						if(data[i].SSID.indexOf("CMCC-") == 0){
							$("#ssidCmcc_ssidpwd_5g").append('<span>CMCC-</span><input type="input" class="col-5" maxlength="27" hgs_key="SSID_5g">(长度:1~32)');
							data[i].SSID = data[i].SSID.substr("CMCC-".length);
							jsonData_5g_tmp = data[i];
						}else{
							$("#ssidCmcc_ssidpwd_5g").append('<input type="input" class="col-5" maxlength="32" hgs_key="SSID_5g">(长度:1~32)')
	...
	for(i in data){
		if (data[i].fullPath) {
			if (data[i].fullPath.indexOf("CMCC_WIFI_BandSteering") > -1) {		
				...
				if (g_staticInfo["Region"] == "Zhejiang" || g_staticInfo["Region"] == "Sichuan") {
					$("#ssidCmcc_allcfg").append('<input type="input" class="col-5" maxlength="32" hgs_key="SSID">(长度:1~32)')
					jsonData = data[i];
				}
				else
				{
					if(data[i].SSID){
						if(data[i].SSID.indexOf("CMCC-") == 0){
							$("#ssidCmcc_allcfg").append('<span>CMCC-</span><input type="input" class="col-5" maxlength="27" hgs_key="SSID">(长度:1~32)');
							data[i].SSID = data[i].SSID.substr("CMCC-".length);
							jsonData = data[i];
						}else{
							$("#ssidCmcc_allcfg").append('<input type="input" class="col-5" maxlength="32" hgs_key="SSID">(长度:1~32)')
	...
	for(i in data){
		if (data[i].fullPath) {
			if (data[i].fullPath.indexOf(PreSharedKeyPath) > -1) {		
				...
				if (g_staticInfo["Region"] == "Zhejiang" || g_staticInfo["Region"] == "Sichuan") {
					$("#ssidCmcc_wifi5").append('<input type="input" class="col-5" maxlength="32" hgs_key="SSID">(长度:1~32)')
					jsonData = data[i];
				}
				else
				{
					if(data[i].SSID){
						if(data[i].SSID.indexOf("CMCC-") == 0){
							$("#ssidCmcc_wifi5").append('<span>CMCC-</span><input type="input" class="col-5" maxlength="27" hgs_key="SSID">(长度:1~32)');
							data[i].SSID = data[i].SSID.substr("CMCC-".length);
							jsonData = data[i];
						}else{
							$("#ssidCmcc_wifi5").append('<input type="input" class="col-5" maxlength="32" hgs_key="SSID">(长度:1~32)')
...
wlanSsidPwdCfgCheckin = function(objName, objData){
	...
	if(ssidNameLen > 32){
		alert("2.4G ssid最多可输入1~32位数字或字符。");
		return;
...
wlanInfInfoCheckout = function(data){
	...
	for(i in data){
		...
		data[i].X_CMCC_APModuleEnable = parseInt(data[i].Enable)?"开启":"关闭";
...
_doWifiScan = function(){
    ...
    function showScanResult(data){
        ...
        innerHtml += "<td>频段</td>";
        innerHtml += "<td>信号强度(dbm)</td>";
        innerHtml += "<td>认证方式</td>";
        innerHtml += "</tr>"
	...
	if (isSubnetAP()) {
		hgsUpdateData(getData,function (data) {
			if (data[0].Status == "Up" || data[0].Status == "ServiceDown") {
				...
				$("#HGS_RELAY #repeatertip").html("WIFI参数已从主网关同步，不允许进行修改");
			}else{
...
pingGetData = function(countdown) {
    hgsUpdateData({type: "GET", url: "/getPingData"}, function(data) {
        ...
        if (countdown > 0) {
            ...
            $("#PingTestData").text("未能在规定时间内接收到完整的ping数据或结束标记!");
        }
    });
}
...
pingCheckin = function(data){
	...
	if ($("#pingProtocol").val() == "IPv6") {
		ipVersion = " -6 ";
		errorMsg = strategies.isValidIPv6(data.Dest, "无效参数:IP地址[" + data.Dest + "]");
	} else if ($("#pingProtocol").val() == "IPv4") {
		ipVersion = " -4 ";
		errorMsg = strategies.isValidIP(data.Dest, "无效参数:IP地址[" + data.Dest + "]");
	} else {
		errorMsg = "Err ipVersion";
	}

	if (errorMsg) {
		errorMsg = strategies.isURL(data.Dest, "无效参数:IP地址或域名[" + data.Dest + "]");
	}
...
voipLinkCheckout = function(data){
	...
	arr.VoipPhoneStatus = (arr.Status == "Up")? "注册成功":"尚未注册成功";
	if(arr.Status == "Up")
	{
		arr.VoipPhoneError = "";
	}else
	{
		switch(arr.X_CMCC_LastRegisterError){
			case "0":
				...
				break;
			case "1":
				arr.VoipPhoneError = "IAD模块错误";
				break;
			case "2":
				arr.VoipPhoneError = "访问路由不通";
				break;
			case "3":
				arr.VoipPhoneError = "访问服务器无响应";
				break;
			case "4":
				arr.VoipPhoneError = "账号错误";
				break;
			case "5":
				arr.VoipPhoneError = "未知错误";
				break;
		}
	}
...
voipQosCfgCheckout = function (data) {
	...
	var innerDSCPHTML = "";
	var inner8021PHTML = "<option value=-1>不修改</option>";
	for (let i = 0; i < 64; i++) {
...
voipProtocolCheckin = function (data) {
	...
	if (reboot) {
		if(confirm("执行该操作后，设备需要'重启'以确保配置数据生效，是否继续?")){
...
dhcpv4PreCheckin = function(data, dhcpData){
	...
	if(MaxAddress <= MinAddress){
		...
		alert("【地址池结束地址】必须大于【地址池起始地址】");
		return true;
	}

	errMsg = strategies.isSameSubnet(dhcpData.SubnetMask, "【地址池结束地址】和【地址池起始地址】不在同一子网!", 
	...
	errMsg = strategies.isSameSubnet(dhcpData.SubnetMask, "【IP地址】和地址池不在同一子网!", 
	...
	if((IPInterfaceIPAddress >= MinAddress) && (IPInterfaceIPAddress <= MaxAddress)){
		...
		alert("LAN侧IP地址不能在地址池的分配区间!")
		return true;
	}
	
	if(dhcpData.IPInterfaceIPAddress != $("#HGS_DHCPV4 [hgs_key=IPInterfaceIPAddress]").attr("oldValue")){
		var r=confirm("你修改了IP地址,修改IP地址会自动重启设备，确认修改【IP地址】吗?");
		if (r!=true){
			return true;
		}
		r=confirm("请确保你的PC或其他终端和新修改的IP [" + dhcpData.IPInterfaceIPAddress + 
			"]在同一个地址段，否则无法正常访问, 确认吗?");
		if (r!=true){
			return true;
		}
		changeIp = true;
