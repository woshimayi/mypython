(function () {
	...
	function failtoUpgardeHanle() {
		$("#promptInfo").html("升级失败!请点击<a href='/' target='_blank'>这里</a>重新到主页面进行升级!");
	}
	...
	function checkUpgradeStatus() {
		...
		$.get("/getUpgradeStatus", function (data) {
			switch (data.retCode) {
				case 0:
					$("#upgradeInfo").text("升级服务进程未响应!");
					failtoUpgardeHanle();
					return;

				case 1:
					$("#upgradeInfo").text("无效的升级文件!");
					failtoUpgardeHanle();
					return;

				case 2:
					$("#upgradeInfo").text("升级文件校验成功!");
					break;

				case 3:
					$("#upgradeInfo").text("正在写入Flash...");
					return;

				case 4:
					$("#upgradeInfo").text("写入flash失败!");
					failtoUpgardeHanle();
					return;

				case 5:
					$("#upgradeInfo").text("写入flash成功!");
					break;

				case 6:
					$("#upgradeInfo").text("正在重启...");
					break;
			}
			setTimeout(checkUpgradeStatus, 2000);
		})
			...
			switch (state) {
				case 0:
					if (firstDisconnectedPromot) {
						...
						$("#context").html('<h3 class="text-danger">网络已经断开!已经无法连接设备!</h3>'
