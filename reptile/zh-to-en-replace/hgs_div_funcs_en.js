...
ponInfoCheckout = function (data) {
	...
	objData.FECSupport    = (objData.FECSupport == "1")?"Supported":"Not supported";
	objData.FECDownstream   = (objData.FECDownstream == "1") ? "Open" : "Close";
...
$("#ServiceList").change(function () {
	...
	hgsUpdateData(getWandata, function (data) {
		...
		if (foundTr069 || foundVoip) {
			...
			alert(serviceList.toString() + "The service has been established, please select other services!");
			if (wanName == "New WAN connection") {
				$("#ServiceList").val("");
			} else {
	...
	if(webPara["VlanEnabled"])
	{
		...
		if(typeof webPara["MulticastVlan"] != "undefined"){
			...
			if((webPara["MulticastVlan"] < 0 && webPara["MulticastVlan"] != -1) || webPara["MulticastVlan"] > 4094){
				...
				alert("Legal range 1-4094, -1 or 0 means not enabled, other values ​​are illegal multicast VLAN!");
				return 1;
			}
...
uplinkCheckin = function(data) {
	...
	if (data.AccessType != "GBE"&& uplinkway == "GBE") {
		//lan上行
		if(confirm("LAN uplink switched to optical uplink, and will be restarted soon to take effect! Are you sure to [Restart Device]?")){
			flag = true;
		}
	} else if (data.AccessType=="GBE"&& uplinkway != "GBE"){
		//GPON上行
		// if(confirm("Optical uplink switched to LAN uplink, the management address will be modified to 192.168.5.1, and will be restarted soon to take effect! Are you sure to [Restart Device]?")){
		if(confirm("Optical uplink switched to LAN uplink, and will be restarted soon to take effect! After restarting, the LAN side address will be changed to 192.168.100.1! Are you sure to [restart the device]?")){
			flag = true;
		}
			...
			if (ed){
				var combobox = $(ed.target).combobox({
					onChange: function(newValue, oldValue){
						...
						if (flag) {
							alert("The bound port cannot be the optical router wired uplink port!");
...
function informStu(value) {
	...
	if (0 == InformStatus) {
		value["InformStatus"] = "received Reporting successfully";
	}	
	else if (3 == InformStatus) {
		value["InformStatus"] = "Reporting process interrupted";
	}
	else if (2 == InformStatus) {
		value["InformStatus"] = "Reporting without response";
	}
	else
	{
		value["InformStatus"] =  "Not reporting";
	}
...
informSelfCheckout = function(value){
	...
	if (0 == informStu) {
		value["Status"] = "received Reporting successfully";
	}	
	else if (3 == informStu) {
		value["Status"] = "Reporting process interrupted";
	}
	else if (2 == informStu) {
		value["Status"] = "Reporting without response";
	}
	else
	{
		value["Status"] =  "Not reporting";
	}
...
function acsStu(value) {
    ...
    if (0 == result)
    {
        value["Result"] =  "Indicates that the service has started to be issued";
    }
    else if (1 == result)
    {
        value["Result"] =  "Service issuance is successful";
    }
    else if (2 == result)
    {
        value["Result"] =  "Service issuance failed";
    }
    else
    {
        value["Result"] =  "No service issuance";
    }	
}	
...
FTPCfgCheckIn = function (data) {
	if (data.FtpEnable) {
		if (isValidLoginPasswd(data.FtpPassword) == false) {
			alert('[Login password] must contain letters, numbers and special characters at the same time, without spaces!');
			return true;
		}
	...
	hgsUpdateData(getData, function (data) {
		...
		hgsUpdateData(postData,function(result){
			if(0 == result[0]){
				...
				alert("Clearing failed! Error code:" + result[0]);
			}
		})
	})
}
	...
	for(row in data){
		...
		for(n in vlanBindList){
			...
			if(2 != vlans.length){
				dg.datagrid('selectRow', row);
				setTimeout(vlanbindUpdate, 1000);
				alert(data[row].vlanBind + "It is an invalid parameter!");
				return true;
			}
			...
			for(i in vlans){
				...
				if(vlan < 1 || vlan > 4094){
					dg.datagrid('selectRow', row);
					setTimeout(vlanbindUpdate, 1000);
					alert(vlan + "It is an invalid VLAN value. The VLAN value must be between [1-4094]!");
					return true;
				}
...
vxLanDelete = function(){
	...
	if(selectRowData.length == 0){
		alert("Please select a line and delete it!");
		return;
	}
...
vxLanCheckin = function(pathStr, data){
	if((($("#VxLanWorkMode").val() == "2")) && ($("#vxLanAddrTypeStatic").is(':checked'))){
		...
		errorMsg = strategies.isSameSubnet(subnetmask, 
			"Invalid parameter: IP address [" + IP + "], subnet mask [" + subnetmask + "], gateway [" + gatewayIp + "] are not in the same network segment!", 
			[IP, gatewayIp]);
		if(errorMsg){
	...
	for (i = 0; i < lanIf.length; i++) {
		...
		if (lanIf[i].Status=="Up") {
			...
			switch(lanIf[i].DuplexMode){
				...
				case "Full":
					lanData[i].DuplexMode = "Full duplex Half";
					break;

				case "Half":
					lanData[i].DuplexMode = "duplex";
					break;
			};
		} else {
			lanData[i].MaxBitRate = "Automatic";
...
macFilterCheckin = function(objName, tableId, data, postData){
	...
	if(tableCheckedData.SourceMACAddress){
		...
		if(!newStandardMac){
			alert(tableCheckedData.SourceMACAddress + "It is an illegal source MAC address");
			return true;
		}
	...
	if(tableCheckedData.DestinationMACAddress){
		...
		if(!newStandardMac){
			alert(tableCheckedData.DestinationMACAddress + "It is an illegal destination MAC address");
			return true;
		}
	...
	if(tableCheckedData.SourceMACAddress == tableCheckedData.DestinationMACAddress){
		alert("The source MAC address and the destination MAC address cannot be the same!");
		return true;
	}
...
ipFilterCheckin = function(objName, tableId, data, postData){
	...
	errMsg = strategies.isEmptyOrValidIP(rowData.SourceIPStart, "Illegal [Source IP (start)]")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isEmptyOrValidIP(rowData.SourceIPEnd, "Illegal [Source IP (end)]")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isEmptyOrValidIP(rowData.DestIPStart, "Illegal [Destination IP (start)]")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isEmptyOrValidIP(rowData.DestIPEnd, "Illegal [Destination IP (end)]")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	...
	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.SourcePortStart, "Illegal [Source Port (start)]", "1:65536")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.SourcePortEnd, "Illegal [Source Port (end)]", "1:65536")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.DestPortStart, "Illegal [Destination Port (start)]", "1:65536")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	errMsg = strategies.isMinMaxIntStringOrEmpty(rowData.DestPortEnd, "[Source Port (end)]", "1:65536")
	if(errMsg){
		alert(errMsg);
		return true;
	}
	...
	if(rowData.SourcePortEnd && rowData.SourcePortStart > rowData.SourcePortEnd){
		alert("must be greater than or equal to [Source Port (start)] [Destination Port (end)");
		return true;
	}
	...
	if(rowData.DestPortEnd && rowData.DestPortStart > rowData.DestPortEnd){
		alert("] must be greater than or equal to [Destination Port (start)]");
		return true;
	}
...
vserverCheckin = function(objName, tableId, data, postData){
	...
	if(rowData.PortMappingDescription.length > 256){
		alert("[Remarks] The length cannot exceed 256 bytes!");
		return true;
	}
	...
	errMsg = strategies.minMaxIntValue(rowData.ExternalPort, "Illegal [External Port], Legal range 1-65555", "1:65535")
	if(errMsg){
		alert(errMsg);
		return true;
	}
...
function intRangeCheck(max, min, range){
	var arr = range.split('-');
	var errMsg = "";
	var errMsg1 = "For illegal [MAX], please enter a value"+range+"a value of an integer between The";
	var errMsg2 = "For illegal [Min], please enter"+range+"a value of an integer between The";
	...
	if(isNaN(max)){
		...
		if(isNaN(min)){
			...
			if (parseInt(max) < parseInt(min)) {
				errMsg = "Max value must not be less than the Min value";
			}
		}
	}
...
qosClassificationTypeCheckin = function (objName, tableId, data, postData) {
	...
	for (let i = 0; i < classifyType_qos.length; i++) {
		if (rowIndex == i) {
			if (hash[tableCheckedData.Type]) {
				alert("The [Match Type] record already exists and cannot be configured repeatedly");
	...
	switch (tableCheckedData.Type) {
		case "WANInterface":
			...
			if (!foundmax) {
				errMsg1 = "[MAX] is illegal, all valid values ​​are listed in its drop-down box, please click the drop-down box and select from it!"
			}
			if (!foundmin) {
				errMsg2 = "[MIN] is illegal, all valid values ​​are listed in its drop-down box, please click the drop-down box and select from it!"
			}
			if (max != min) {
				errMsg1 = "When the match type is WANInterface, the [min] and [max] values ​​should be the same"
			}
			break;
		case "LANInterface":
		...
		case "SIP":
		case "DIP":
			errMsg1 = strategies.isEmptyOrValidIP(max, "[MAX] IP address Illegal [Min] IP address");
			errMsg2 = strategies.isEmptyOrValidIP(min, "[MAX] IP address");
			err1 = strategies.isValidIPv6(max, "[MAX] IP address Illegal [Min] IP address");
			err2 = strategies.isValidIPv6(min, "[MAX] IP address");
			...
			if (!errMsg1 && !errMsg2) {
				...
				for (let i = 0; i < arr1.length; i++) {
					if (parseInt(arr1[i]) < parseInt(arr2[i])) {
						errMsg1 = "must not be less than [Min] IP address [MAX";
					}
				}
			}
			else if (!err1 && !err2){ 
				...
				if ((!errMsg1 || !err1) && (!errMsg2 || !err2)) {
					errMsg1 = "] and [Min] should be the same IPv4 or IPv6 address";
				}
			}
			break;
		case "SPORT":
	...
	if (ProtocolList == "") {
		...
		for (var k in procArr) {
			if (proclist.indexOf(procArr[k]) == -1) {
				alert("[Protocol] is illegal, please click to select a valid protocol value from the drop-down box!");
				return true;
			}
		}
	}
...
function qosClassificationTypeEditRow(index, checkRow){
	...
	$(ed.target).combobox({
		...
		onChange:function (newValue, oldValue) {
			...
			if (!found) {
				alert("All valid values ​​are listed in the drop-down box. Please click the drop-down box and select from it! Please select a");
...
qosAppCreate = function () {
	...
	if(rowsData.length > 0){
		if (rowsData.length == 2) {
			alert("! The service is repeated, please reset it");
			return;
		}
		if(!rowsData[rowsData.length - 1].fullPath){
			alert("Please select a")
...
priorityQueueWeightCheckin = function (objName, data) {
	...
	if (WeightSum != 100) {
		alert("Please select a row and save! The");
		return true;
	}
	...
	switch (data.SpeedLimitMode_UP) {
		case 1:
			...
			for (var i = 0; i < speed.length; i++) {
				...
				if (element.length != 2) {
					alert("is illegal! The rate is illegal, please re-enter");
					return;
				} else {
					if ($.inArray(element[0], interfaces) == -1) {
						alert(", please re-enter! The user interface is illegal");
						return;
					}
		...
		case 3:
			...
			for (var i = 0; i < speed.length; i++) {
				...
				if (element.length != 2) {
					...
					if (ip.length != 2) {
						alert("The ip is illegal");
					} else {						
						var err1 = strategies.isValidIP(ip[0],"The ip address is illegal! The ip address");
						var err2 = strategies.isValidIPv6(ip[0],"The ip address is illegal! The ip address");
						var err3 = strategies.isValidIP(ip[1],"The ip address is illegal! The ip address");
						var err4 = strategies.isValidIPv6(ip[1],"The ip address is illegal! The ip address");
						if ((err1 && err2) || (!err1 && err3) || (!err2 && err4)) {
							alert("The ip address is illegal! The ip address");
							return;
						}						
					}
...
userAccountCheckin = function(path, data){
	...
	if(para.ConfirmUserNewPwd.length == 0){
		alert("New User Password] cannot be empty! [Confirm New User Password] cannot be empty");
		$('#HGS_USER_ACCOUNT [hgs_key="ConfirmUserNewPwd"]').focus();
		return true;
	}
	if(para.UserNewPassword != para.ConfirmUserNewPwd){
		$('#HGS_USER_ACCOUNT [hgs_key="ConfirmUserNewPwd"]').focus();
		alert("! [New User Password] and [Confirm New User Password] are inconsistent!");
		return true;
	}
	...
	if (g_staticInfo["Region"] != "Sichuan") {
		if(para.UserNewPassword == para.UserOldPassword){
			$('#HGS_USER_ACCOUNT [hgs_key="UserNewPassword"]').focus();
			alert("[New User Password] and [Old User Password] cannot be exactly the same!");
			return true;
		}
	}

	if (para.ConfirmUserNewPwd.length < 8) {
		alert('The length of [New User Password] cannot be less than 8 characters!');
		return true;		
	}

	if (isValidLoginPasswd(para.UserNewPassword) == false) {
		alert('[New User Password] must contain letters, numbers and special characters, without spaces!');
		return true;
	}
	...
	switch(data.retCode){
		case 0:
			if(g_adminAccount && g_modifyUserAccount){
				...
				alert("Password modified successfully, about to jump to the login page!");
			}
			break;
		case 1:
			alert("[Old User Password] Error!");
			flag = true;
			break;
		case 2:
			alert("[New User Password] and [Old User Password] cannot be exactly the same!");
...
rebootCheckIn = function(data){
	if(confirm("The device is about to restart! It takes about 50 seconds to restart the device. Are you sure to [restart the device]?")){
		return;
	}
...
rebootResp = function(data){
	if(0 == data.retCode){
		...
		//alert("The device is restarting, please wait patiently for 40 seconds!");
	}else{
		alert("Failed to restart the device! Error code:" + data.retCode);
	}
}
...
restoreDefResp = function(data){
	if(0 == data.retCode){
		alert("The default configuration is restored successfully! The device is restarting, please wait patiently for 40 seconds!");
	}else{
		alert("Failed to restore the default configuration! Error code:" + data.retCode);
	}
}
...
restoreFacResp = function(data){
	if(0 == data.retCode){
		alert("Factory settings restored successfully! Restarting the device, please wait patiently for 40 seconds!");
	}else{
		alert("Factory settings restored failed! Error code:" + data.retCode);
	}
}
/*----------------------------------------------------------------------------*/
	...
	switch($("#OnuAuthType").val()){
		case "1":
			if (passwd.length > len) {
				alert("The password length is at least 1 character and at most"+len+"characters!");
				return;
			}
			data.UserName = "";
			break;
...
staticRouteCheckin = function(objName, data){
	...
	for(var i in data){
		...
		errMsg = strategies.isValidIP(data[i].DestIPAddress, data[i].DestIPAddress + "Illegal [Destination IP]")
		if(errMsg){
			alert(errMsg);
			return;
		}
...
wlanSsidPwdCfgCheckin = function(objName, objData){
	...
	if(ssidNameLen > 32){
		alert("2.4G ssid can be entered with a maximum of 1~32 digits or characters. 2.4G ssid");
		return;
	}else if(ssidNameLen == 0){
		alert("cannot be empty. 5G ssid can be entered with a maximum of 1~");
		return;
	}

	if(ssidNameLen_5g > 32){
		alert("32 digits or characters. 5G ssid cannot be empty. ssid can be entered with a maximum of 1~32 digits or characters.");
		return;
...
wlanAllCfgCheckin = function(objName, objData){
	...
	if(ssidNameLen > 32){
		alert("ssid can be entered with a maximum of 1~32 digits or characters.");
		return;
	}
	if(ssidNameLen == 0){
		alert("ssid cannot be empty.");
...
wlanInfInfoCheckout = function(data){
	...
	for(i in data){
		...
		data[i].X_CMCC_APModuleEnable = parseInt(data[i].Enable)?"Open":"Close";
...
$("#hgc_repeaterCfg").click(function(event){
    ...
    if ( !$("#hgc_enableRelay").is(':checked') ) {
		alert("Please open wireless relay first.");
		return;
    } 
	...
	if (isSubnetAP()) {
		hgsUpdateData(getData,function (data) {
			if (data[0].Status == "Up" || data[0].Status == "ServiceDown") {
				...
				$("#HGS_RELAY #repeatertip").html("WIFI parameters have been synchronized from the main gateway and cannot be modified.");
			}else{
...
pingGetData = function(countdown) {
    hgsUpdateData({type: "GET", url: "/getPingData"}, function(data) {
        ...
        if (countdown > 0) {
            ...
            $("#PingTestData").text("Failed to receive complete ping data or end mark within the specified time!");
        }
    });
}
...
voipLinkCheckout = function(data){
	...
	arr.VoipPhoneStatus = (arr.Status == "Up")? "Registration successful":"Not yet registered";
	if(arr.Status == "Up")
	{
		arr.VoipPhoneError = "";
	}else
	{
		switch(arr.X_CMCC_LastRegisterError){
			...
			case "1":
				...
				break;
			case "2":
				arr.VoipPhoneError = "Access to route unavailable";
				break;
			case "3":
				arr.VoipPhoneError = "Access to server unresponsive";
				break;
			case "4":
				arr.VoipPhoneError = "Account error";
				break;
			case "5":
...
voipProtocolCheckin = function (data) {
	...
	if (reboot) {
		if(confirm("After performing this operation, the device needs to be 'restarted' to ensure that the configuration data takes effect. Do you want to continue?")){
...
dhcpv4PreCheckin = function(data, dhcpData){
	...
	errMsg = strategies.isSameSubnet(dhcpData.SubnetMask, "[IP address] and address pool are not in the same subnet!", 
	...
	if((IPInterfaceIPAddress >= MinAddress) && (IPInterfaceIPAddress <= MaxAddress)){
		...
		alert("LAN side IP address cannot be in the allocation interval of the address pool!")
		return true;
	}
	
	if(dhcpData.IPInterfaceIPAddress != $("#HGS_DHCPV4 [hgs_key=IPInterfaceIPAddress]").attr("oldValue")){
		var r=confirm("You have modified the IP address, and the device will automatically restart after modifying the IP address. Confirm the modification of [IP address]?");
		if (r!=true){
			return true;
		}
		r=confirm("Make sure your PC or other terminal and the newly modified IP [" + dhcpData.IPInterfaceIPAddress + 
			"] are in the same address segment, otherwise it cannot be accessed normally. Confirm?");
		if (r!=true){
			return true;
		}
		changeIp = true;
...
mirrorCfgCheckin = function(){
	...
	if((!mirrorCfg.source) && (!mirrorCfg.wanMirrorEnable)){
		alert("Mirror source port not selected!");
		return true;
	}

	if(!mirrorCfg.dest){
		alert("Mirror destination port not selected!");
		return true;
	}

	if(mirrorCfg.source == mirrorCfg.dest){
		alert("Mirror source port and destination port cannot be the same port!");
		return true;
	}
...
function errorCfgFile(e){
	var strInfo = "Upload file error! Error code:" + e.status +", Error description:" + e.statusText;
