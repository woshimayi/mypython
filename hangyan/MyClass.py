...
main_url   ='http://192.168.1.1/cgi-bin/content.asp'
...
class autoTest(object):
	...
	def loginUser(self, loginurl=login_url, mainurl=main_url, username=user_name, passwd=pass_wd):
		print('=====login=====')
		...
		if loginurl == self.driver.current_url:   #获取当前页面的网址
			...
			self.driver.find_element_by_id("Password").send_keys(pass_wd)   #在对象上模拟按键键入
			self.driver.find_element_by_id("btnSubmit").click()   #点击
			time.sleep(0.5)

		if main_url != self.driver.current_url:   #获取当前页面的网址
			self.driver.find_element_by_id('Username').clear()  #清理
			self.driver.find_element_by_id('Username').send_keys(user_name)
			self.driver.find_element_by_id("Password").clear()   #清除对象的内容
			self.driver.find_element_by_id("Password").send_keys(pass_wd)   #在对象上模拟按键键入
	...
	def local_kernel_web_update(self):
		...
		try:
			...
			win32gui.SendMessage(Edit, win32con.WM_SETTEXT, None, '/home/zs/Documents/mypython/hangyan/tclinux.bin')  # 往输入框输入绝对地址
