...
class autoTest(object):
	...
	def loginUser(self, url=None, mainurl=None, username=None, passwd=None):
		...
		if url == driver.current_url:   #获取当前页面的网址
			...
			driver.find_element_by_id('LOGIO_TEXT_UserName').send_keys(self.username)
			driver.find_element_by_id("LOGIO_PWD_Password").clear()   #清除对象的内容
			driver.find_element_by_id("LOGIO_PWD_Password").send_keys(self.passwd)   #在对象上模拟按键键入
	...
	def pingWan(self, DNS = 'www.baidu.com'):
		try:
			...
			self.driver.find_element_by_id('MENU_THD_DiagNetPing').click()   #poing测试
			time.sleep(0.5)
